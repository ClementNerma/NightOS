...
Note that some filesystems may not support this feature. For those, the FEID will be derived from the item's [split path](services/integration/filesystem-interfaces.md#split-paths), which means it will still be unique in the filesystem but will not be resilient to file moving or another file replacing the previous one.

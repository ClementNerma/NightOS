...
Even if the process closes its CMDMSG or CMDRAW pipe properly (by calling the [`CLOSE_PIPE`](syscalls.md#0x26-close_pipe)), the command is not considered as finished until the process itself did not terminate.
